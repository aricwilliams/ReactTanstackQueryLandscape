...
export function IssueItem({
  ...
  queryClient.prefetchInfiniteQuery(
    ...
    () => fetchWithError(`/api/issues/${number}/comments?page=1`)
              ...
              <th scope="row"><a href="#">{status === "done" || status === "cancelled" ? (
    <GoIssueClosed style={{ color: "red" }} />
  ) : (
    <GoIssueOpened style={{ color: "green" }} />
  )}</a></th>
              <td className="issue-content"><span>
  ...
  <small>
    ...
    {createdByUser.isSuccess ? `by ${createdByUser.data.name}` : ""}
              ...
              <td><a href="#" className="text-primary">{assignee ? (
  <img
    src={
  ...
  />
    ...
    <>
      ...
      {commentCount}
              ...
              <td><span className="issue-content">{labels.map((label) => (
